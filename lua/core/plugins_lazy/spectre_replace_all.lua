return {
	{
		...
		config = function()
			...
			spectre.setup({
				...
				mapping = {
					-- ["u"] = {
					-- 	map = "u",
					-- 	cmd = "<cmd>undo<CR>", -- or just "undo" command
				...
				replace_engine = {
					...
					["oxi"] = { cmd = "oxi", args = {}, options = { ["ignore-case"] = { value = "i", icon = "[I]", desc = "ignore case" } } },
					["sd"] = { cmd = "sd", options = {} },
				},
			...
			keymap.set("n", "<C-j>", ":lua require('spectre').open()<CR>", opts)
			keymap.set(
				"n",
				"<leader>rw",
			...
			keymap.set("n", "<leader>W", ":lua require('spectre').open()<CR>", opts)
		end,
		keys = {
			{ "<C-j>", mode = "n" },
			{ "<leader>rw", mode = { "n", "v" } },
		...
		cmd = "Spectre", -- also load on command
	},
}
