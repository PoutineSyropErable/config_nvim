...
local function get_source_directories()
	...
	if cwd == "/" or cwd == home then
		vim.notify("[WARN] get_source_directories() aborted: Unsafe directory")
		return {}
	end
	...
	if #subdirs < 3 then
		vim.notify("[WARN] get_source_directories() aborted: Not deep enough")
		return {}
	end
...
local function get_class_path()
	...
	if file_ext ~= "java" then
		vim.notify("❌ (In get_class_path), Error: This is not a Java file!")
		return nil
	end
	...
	if result == "" then
		vim.notify("❌ Error: `dap-utils.py` did not return a classpath!")
		return nil
	end
...
local function get_main_class()
	...
	local file_ext = vim.fn.fnamemodify(java_file_path, ":e")
	if file_ext ~= "java" then
		vim.notify("❌ (in get_main_class) Error: This is not a Java file!")
		return nil
	end
	...
	-- Handle errors (empty output means something went wrong)
	if result == "" then
		vim.notify("❌ Error: `dap-utils.py` did not return a main class!")
		return nil
	end
...
local function wait_for_debugger_async(host, port, timeout, callback)
	...
	vim.notify("⏳ Waiting for Java Debugger to start on " .. host .. ":" .. port .. "...")
	...
	local function check_debugger()
		...
		socket:connect(host, port, function(err)
			if not err then
				socket:close()
				vim.notify("✅ Java Debugger is ready! Attaching...")
				callback(true)
			else
				if (uv.now() - start_time) >= timeout then
					vim.notify("❌ Timeout! Java Debugger did not start within " .. timeout .. "ms.")
					callback(false)
				else
...
local function start_automake()
	...
	if java_file == "" then
		vim.notify("❌ No Java file selected!")
		return
	end
	...
	vim.fn.jobstart(cmd, {
		on_exit = function(_, code)
			if code == 0 then
				vim.notify("✅ Automake started Java Debugger.")
			else
				vim.notify("❌ Automake failed.")
			end
		end,
	})
end
...
vim.keymap.set("n", "<leader><F8>", function()
	...
	if class_path then
		vim.notify("✅ Java Class Path: " .. table.concat(class_path, "\n"))
	end
...
vim.keymap.set("n", "<leader><F9>", function()
	...
	if main_class then
		vim.notify("✅ Java Main Class: " .. main_class)
	end
